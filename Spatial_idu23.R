...
WW<-mat2listw(matrix,style = "W") #row-normalized weights
...
#distance base tests - row-normalized binary  weights; zero.policy=TRUE must be present since there is always points with no (living) neighbours
...
tabela[2,4]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(dnearneigh(parcela_vivas, d1=0, d2=5),zero.policy = TRUE,style="B"),zero.policy=TRUE)$estimate[1],3)
tabela[2,5]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(dnearneigh(parcela_vivas, d1=0, d2=5),zero.policy = TRUE,style="B"),zero.policy=TRUE)$p.value,5)
tabela[3,4]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(dnearneigh(parcela_vivas, d1=0, d2=6),zero.policy = TRUE,style="B"),zero.policy=TRUE)$estimate[1],3)
tabela[3,5]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(dnearneigh(parcela_vivas, d1=0, d2=6),zero.policy = TRUE,style="B"),zero.policy=TRUE)$p.value,5)
tabela[4,4]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(dnearneigh(parcela_vivas, d1=0, d2=7),zero.policy = TRUE,style="B"),zero.policy=TRUE)$estimate[1],3)
tabela[4,5]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(dnearneigh(parcela_vivas, d1=0, d2=7),zero.policy = TRUE,style="B"),zero.policy=TRUE)$p.value,5)
#distance base tests - Row-normalized idw weights; can be used nb2listwdist function
tabela[2,6]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=5),parcela_vivas,type="idw",style="W", zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[2,7]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=5),parcela_vivas,type="idw",style="W", zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
tabela[3,6]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=6),parcela_vivas,type="idw",style="W", zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[3,7]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=6),parcela_vivas,type="idw",style="W", zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
tabela[4,6]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=7),parcela_vivas,type="idw",style="W", zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[4,7]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=7),parcela_vivas,type="idw",style="W", zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
#distance base tests - idw weights; can be used nb2listwdist function
tabela[2,8]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=5),parcela_vivas,type="idw", zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[2,9]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=5),parcela_vivas,type="idw", zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
tabela[3,8]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=6),parcela_vivas,type="idw", zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[3,9]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=6),parcela_vivas,type="idw", zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
tabela[4,8]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=7),parcela_vivas,type="idw", zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[4,9]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=7),parcela_vivas,type="idw", zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
...
tabela[5,4]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(knn2nb(knearneigh(parcela_vivas, k=4)),style="B"),zero.policy=TRUE)$estimate[1],3)
tabela[5,5]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(knn2nb(knearneigh(parcela_vivas, k=4)),style="B"),zero.policy=TRUE)$p.value,5)
tabela[6,4]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(knn2nb(knearneigh(parcela_vivas, k=8)),style="B"),zero.policy=TRUE)$estimate[1],3)
tabela[6,5]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listw(knn2nb(knearneigh(parcela_vivas, k=8)),style="B"),zero.policy=TRUE)$p.value,5)
#k neighbours - row-normalized inverse of distance weights 
tabela[5,6]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=4)),parcela_vivas,type="idw",style="W",zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[5,7]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=4)),parcela_vivas,type="idw",style="W",zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
tabela[6,6]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=8)),parcela_vivas,type="idw",style="W",zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[6,7]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=8)),parcela_vivas,type="idw",style="W",zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
#k neighbours - inverse of distance weights 
tabela[5,8]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=4)),parcela_vivas,type="idw",zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[5,9]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=4)),parcela_vivas,type="idw",zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
tabela[6,8]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=8)),parcela_vivas,type="idw",zero.policy = TRUE),zero.policy=TRUE)$estimate[1],3)
tabela[6,9]<-round(moran.test(parcela_vivas$du_annual_growth,nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=8)),parcela_vivas,type="idw",zero.policy = TRUE),zero.policy=TRUE)$p.value,5)
#Area of influence neighbours - three weights (WW,  W, WdistW, Wdist)
...
tabela[7,4]<-round(moran.test(parcela_vivas$du_annual_growth,W, zero.policy=TRUE)$estimate[1],3)
tabela[7,5]<-round(moran.test(parcela_vivas$du_annual_growth,W, zero.policy=TRUE)$p.value,5)
tabela[7,6]<-round(moran.test(parcela_vivas$du_annual_growth,Wdist, zero.policy=TRUE)$estimate[1],3)
tabela[7,7]<-round(moran.test(parcela_vivas$du_annual_growth,Wdist, zero.policy=TRUE)$p.value,5)
tabela[7,8]<-round(moran.test(parcela_vivas$du_annual_growth,WdistW, zero.policy=TRUE)$estimate[1],3)
tabela[7,9]<-round(moran.test(parcela_vivas$du_annual_growth,WdistW, zero.policy=TRUE)$p.value,5)
nomes<-c("Weights","Row-normalized Binary","","Binary","", "Row-normalized idw","","idw");colnames(tabela)<-nomes
#tabelaA<-tabela #save the table
################
#Choosen and locked spatial weights matrices to use for modelling
listw_A<-nb2listw(dnearneigh(parcela_vivas, d1=0, d2=6),zero.policy = TRUE,style="W")
listw_B<-nb2listwdist(dnearneigh(parcela_vivas, d1=0, d2=7),parcela_vivas,type="idw",style="W",zero.policy = TRUE)
listw_C<-nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=4)),parcela_vivas,type="idw",style="W")
listw_D<-nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=4)),parcela_vivas,type="idw",style="W") 
listw_ABCD<-nb2listwdist(knn2nb(knearneigh(parcela_vivas, k=4)),parcela_vivas,type="idw",style="W") 
######
#Evaluation: 
#Raw distances tend to be better, but to follow model assumptions we select on row-normalized weights
#Neither distance based or k-neighbours are better, depends on the site itself
#Area of Influence approach seems to work better on site A and B, where tree size is more regular
#Area of Influence approach may overestimate the neighbours in larger trees, underestimate in very small trees (very few or no neighbours) in C and D
#Also Lack of representation of the plantation makes Area of Influence approach not adequate
########
...
plot(sp.correlogram(dnearneigh(parcela_vivas, d1=0, d2=5), parcela_vivas$du_annual_growth, method="I" ,order=9,zero.policy=TRUE),ylim=c(0,0.35), main="dist 5m")
plot(sp.correlogram(dnearneigh(parcela_vivas, d1=0, d2=6), parcela_vivas$du_annual_growth, method="I" ,order=9,zero.policy=TRUE),ylim=c(0,0.35), main="dist 6m")
plot(sp.correlogram(dnearneigh(parcela_vivas, d1=0, d2=7), parcela_vivas$du_annual_growth, method="I" ,order=9,zero.policy=TRUE),ylim=c(0,0.35) ,main="dist 7m")
...
#Lock correlograms for each plant
par(mfrow=c(1,1))
...
correl_c<-plot(sp.correlogram(dnearneigh(parcela_vivas, d1=0, d2=7), parcela_vivas$du_annual_growth, method="I" ,order=30,zero.policy=TRUE),ylim=c(0,0.40) ,main="C")#needs more lags
...
#Check the scores from Lagrange Modifier tests and SARMA

lm.LMtests(model,listw_A, zero.policy = TRUE, test="all")
lm.LMtests(model,listw_B, zero.policy = TRUE, test="all")
lm.LMtests(model,listw_C, zero.policy = TRUE, test="all")
lm.LMtests(model,listw_D, zero.policy = TRUE, test="all")
lm.LMtests(model,listw_ABCD, zero.policy = TRUE, test="all")
...
LR.Sarlm(spatiallag,model) #SLM vs OLS
LR.Sarlm(spatialerr,model) #SEM vs OLS
...
#A
#spatialerrA<-errorsarlm(du_annual_growth~Cea_0.5m_1px+Cea_1m_1px+TWI_tree_1px,data=parcela_vivas,listw=listw_A,zero.policy = TRUE) #OLS 
spatialerrA<-errorsarlm(du_annual_growth~Cea_0.5m_1px+Cea_1m_1px+TWI_tree_1px,data=parcela_vivas,listw=listw_A,zero.policy = TRUE)
summary(spatialerrA)
AIC(spatialerrA)
round(1-(sum((parcela_vivas$du_annual_growth-(spatialerrA$fitted.values))^2)/sum((parcela_vivas$du_annual_growth-mean(parcela_vivas$du_annual_growth))^2)),4) #R2 
Ypred<-fitted(model) ; plot(Ypred~parcela_vivas$du_annual_growth)
moran.test(spatialerrA$residuals,listw_A ,zero.policy = TRUE) #check spatial autocorrelation from residuals
#B
#spatialSACB<-sacsarlm(model,data=parcela_vivas,listw=listw_B,zero.policy = TRUE)
spatialSACB<-sacsarlm(du_annual_growth~slope_1px+TRI_1px,data=parcela_vivas),data=parcela_vivas,listw=listw_B,zero.policy = TRUE)
summary(spatialSACB)
round(1-(sum((parcela_vivas$du_annual_growth-(spatialSACB$fitted.values))^2)/sum((parcela_vivas$du_annual_growth-mean(parcela_vivas$du_annual_growth))^2)),4)
Ypred<-fitted(spatialSACB) ; plot(Ypred~parcela_vivas$du_annual_growth)
moran.test(spatialSACB$residuals,listw_B ,zero.policy = TRUE)                
#C
spatialSACC<-sacsarlm(lm(du_annual_growth~Cea_0.5m_1px+Altimetria_1px,data=parcela_vivas),data=parcela_vivas,listw=listw_C,zero.policy = TRUE)
summary(spatialSACC)
round(1-(sum((parcela_vivas$du_annual_growth-(spatialSACC$fitted.values))^2)/sum((parcela_vivas$du_annual_growth-mean(parcela_vivas$du_annual_growth))^2)),4)
Ypred<-fitted(spatialSACC) ; plot(Ypred~parcela_vivas$du_annual_growth)
moran.test(spatialSACC$residuals,listw_C ,zero.policy = TRUE)
#D
spatialSACD<-sacsarlm(model,data=parcela_vivas,listw=listw_D,zero.policy = TRUE)
summary(spatialSACD)
round(1-(sum((parcela_vivas$du_annual_growth-(spatialSACD$fitted.values))^2)/sum((parcela_vivas$du_annual_growth-mean(parcela_vivas$du_annual_growth))^2)),4)
Ypred<-fitted(spatialSACD) ; plot(Ypred~parcela_vivas$du_annual_growth)
moran.test(spatialSACD$residuals,listw_D ,zero.policy = TRUE)
#ABCD
spatialSACABCD<-sacsarlm(
  lm(du_annual_growth~Cea_0.5m_1px+Altimetria_1px+slope_1px,data=parcela_vivas),
  data=parcela_vivas,listw=listw_tudo,zero.policy = TRUE)
summary(spatialSACABCD)
Ypred<-fitted(spatialSACABCD)
plot(Ypred~parcela_vivas$du_annual_growth)
round(1-(sum((parcela_vivas$du_annual_growth-(spatialSACABCD$fitted.values))^2)/sum((parcela_vivas$du_annual_growth-mean(parcela_vivas$du_annual_growth))^2)),4)
